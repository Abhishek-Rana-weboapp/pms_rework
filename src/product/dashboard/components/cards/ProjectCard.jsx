...
const ProjectCard = ({ project, onClick }) => {
  return (
    <div
      ...
      className="w-full rounded-md shadow bg-white p-4 space-y-3 cursor-pointer hover:scale-[1.01] transition"
    >
      ...
      <div className="flex justify-between items-center w-full">
        <AvatarGroup>
          {project?.developers_details?.length > 0 ? (
        ...
        <span className="text-xs text-gray-500">{project.end_date}</span>
      </div>
    </div>
  );
};
