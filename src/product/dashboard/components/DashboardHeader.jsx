...
import { DropdownMenu, DropdownMenuContent, DropdownMenuGroup, DropdownMenuItem, DropdownMenuSeparator, DropdownMenuTrigger } from "@/shared/components/ui/dropdown-menu";
...
const DashboardHeader = () => {
    ...
    const segment = location.pathname.replace(`/${orgUuid}`, "").replace(/^\//, "");
    const segmentSplit = segment.split("/")[0]
    const currentTitle = SideBarItems.find((item) => item.to === segmentSplit)?.title ||  "";

    console.log(user);
    
  return <header className="w-full py-3 shadow px-2 border-b">
                  ...
                  <DropdownMenuContent className={"w-max max-w-76"}>
                    <DropdownMenuGroup>
                          <div className="flex items-center gap-2 px-2.5 py-1">
                            <Avatar>
                                 <AvatarImage src={user?.user_image}/>
                                 <AvatarFallback className={"text-black"}>{createInitials(user)}</AvatarFallback>
                            </Avatar>
                    ...
                    <DropdownMenuSeparator />
                       <DropdownMenuGroup>
                           <DropdownMenuItem className={"  text-red-500"}>
                               <button className={"flex justify-center items-center gap-1"} onClick={logout}><Power/>Sign Out</button>
                           </DropdownMenuItem>
                       </DropdownMenuGroup>
                  </DropdownMenuContent>
              </DropdownMenu>
